...
pygame.init()
pygame.mixer.init()
pygame.mixer.music.load("cheeky_monkey_fun_app_playful_cheeky.mp3")
...
# 사용자 닉네임 입력을 받기 위한 TEXT INPUT 모듈
class TextInput:
    def __init__(
            self,
            initial_string="",
    ...
    while intro:
        ...
        for event in events:
            if event.type == pygame.QUIT:
                pygame.quit()
            if event.type == pygame.KEYDOWN and len(textinput.get_text()) > 0 and event.key == pygame.K_RETURN:
                intro = False
...
def draw_time(start_time,pause_time):
    ...
    return [time_font.render(get_time(current_time), True, white, color), get_time(current_time)]
...
def rank_print():
    ...
    rank1_time = rank_font.render(get_time(sdict[0][1]), True, color)
    ...
    rank2_time = rank_font.render(get_time(sdict[1][1]), True, color)
    ...
    rank3_time = rank_font.render(get_time(sdict[2][1]), True, color)
    ...
    rank4_time = rank_font.render(get_time(sdict[3][1]), True, color)
    ...
    rank5_time = rank_font.render(get_time(sdict[4][1]), True, color)
    ...
    while not done:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN and victory:
                    done = True
        ...
        if not victory and not pause:
            ...
            if goal.colliderect((x, y, player_size, player_size)):
                victory = True
                round += 1
                sum += time.time() - start - pause_time
